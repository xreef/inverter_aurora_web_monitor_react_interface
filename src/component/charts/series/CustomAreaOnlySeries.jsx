...
class CustomAreaOnlySeries extends AreaOnlySeries {
    constructor(props) {
        ...
        const {textureImageSrc} = props;

        if (textureImageSrc) {
            loadImage(textureImageSrc, (img) => {
                this.setState({textureImage: img});
            });
        }
    }
    drawOnCanvas(ctx, moreProps) {
        const { yAccessor, defined, reference, refPriceOnPlottedData } = this.props;
        const { fill, stroke, opacity, textureImageSrc } = this.props;

        const { xScale, chartConfig: { yScale }, plotData, xAccessor } = moreProps;

        let yn = getReferenceHeight(((refPriceOnPlottedData)?((plotData && plotData.length>0)?plotData[0].close:0):reference), yScale.domain()[0], yScale.domain()[1], yScale.range()[0], yScale.range()[1]);
        ...
        if ((textureImageSrc==='undefined' || textureImageSrc===null) || (textureImageSrc && this.state.textureImage)){
            let points0 = [], points1 = [];

            for (let i = 0; i < plotData.length; i++) {
                let d = plotData[i];
                if (defined(yAccessor(d), i)) {
                    let [x, y1, y0] = [xScale(xAccessor(d)), yScale(yAccessor(d)), yn];
    ...
    renderSVG(moreProps) {
        const { yAccessor, defined, reference, refPriceOnPlottedData } = this.props;
        let { stroke, fill, className, opacity, textureImageSrc } = this.props;

        const { xScale, chartConfig: { yScale }, plotData, xAccessor } = moreProps;

        let yn = getReferenceHeight(((refPriceOnPlottedData)?((plotData && plotData.length>0)?plotData[0].close:0):reference), yScale.domain()[0], yScale.domain()[1], yScale.range()[0], yScale.range()[1]);
        let areaSeries = d3Area()
            .defined(d => defined(yAccessor(d)))
            .x((d) => xScale(xAccessor(d)))
            .y0(yn)
            .y1((d) => yScale(yAccessor(d)));

        let d = areaSeries(plotData);
        ...
        let gradientName = 'gradientMWArea-'+guid();
        let patternName = 'patternMWArea-'+guid();

        let gradient = (fill.middleUp && fill.middleDown)?(
        ...
        return <g>
            {(this.state.textureImage )?(
                (true)?(
					<defs>
						<pattern id={patternName} patternUnits='userSpaceOnUse' width={this.state.textureImage.width}
								 height={this.state.textureImage.height}>
							<image href={textureImageSrc}/>
						</pattern>
                        {gradient}
...
CustomAreaOnlySeries.propTypes = {
    ...
    textureImageSrc: PropTypes.string,
