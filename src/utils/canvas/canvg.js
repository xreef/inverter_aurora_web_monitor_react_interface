/*
 * Refactoring to use as import without add file as external dependencies
 *
...
let canvg = function (target, s, opts) {
    // no parameters
...
if (nodeEnv) {
    ...
    matchesSelector = function (node, selector) {
        let styleClasses = node.getAttribute('class');
...
function build(opts) {
    ...
    svg.parseXml = function (xml) {
        if (typeof Windows != 'undefined' && typeof Windows.Data != 'undefined' && typeof Windows.Data.Xml != 'undefined') {
            ...
            try {
                let parser = new windowEnv.DOMParser();
    ...
    svg.Transform = function (v) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            let transformType = this.Type[type];
            if (typeof transformType != 'undefined') {
                let transform = new transformType(s);
                transform.type = type;
    ...
    svg.Element.ElementBase = function (node) {
        ...
        this.render = function (ctx) {
            ...
            if (this.style('mask').hasValue()) { // mask
                let mask = this.style('mask').getDefinition();
        ...
        // Microsoft Edge fix
        let allUppercase = new RegExp("^[A-Z\-]+$");
        let normalizeAttributeName = function (name) {
        ...
        if (node != null && node.nodeType == 1) { //ELEMENT_NODE
            ...
            for (let i = 0; i < node.attributes.length; i++) {
                ...
                let nodeName = normalizeAttributeName(attribute.nodeName);
    ...
    svg.Element.RenderedElementBase = function (node) {
        ...
        this.setContext = function (ctx) {
            ...
            if (this.style('transform', false, true).hasValue()) {
                let transform = new svg.Transform(this.style('transform', false, true).value);
                transform.apply(ctx);
            }

            // clip
            if (this.style('clip-path', false, true).hasValue()) {
                let clip = this.style('clip-path', false, true).getDefinition();
    ...
    svg.Element.GradientBase = function (node) {
        ...
        this.inheritStopContainer = function (stopsContainer) {
            for (let i = 0; i < this.attributesToInherit.length; i++) {
                let attributeToInherit = this.attributesToInherit[i];
    ...
    svg.Element.clipPath = function (node) {
        ...
        this.apply = function (ctx) {
            let hasContext2D = (typeof CanvasRenderingContext2D !== 'undefined');
            let oldBeginPath = ctx.beginPath;
            let oldClosePath = ctx.closePath;
            if (hasContext2D) {
